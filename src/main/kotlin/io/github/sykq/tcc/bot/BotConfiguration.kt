...
//                session.textMessage("sykq", "connected")
            ...
            override fun onMessage(session: TmiSession, messages: Flux<TmiMessage>): Mono<Void> {
                return messages.log()
                    .doOnEach { totalMessages.getAndIncrement() }
                    .filter { it.tags["subscriber"]?.contains("1") ?: false }
                    .doOnEach { subscriberMessages.getAndIncrement() }
                    .then()
            }
